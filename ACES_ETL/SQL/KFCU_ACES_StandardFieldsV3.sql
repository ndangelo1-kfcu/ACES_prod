 ...
 --------       ELSE lp.[Property Zip]
	...
	, UPPER(CASE 
	 ...
	 ,	UPPER( 
				REPLACE(
							(
								CASE WHEN Funder.ReportsToName like 'Carrie Graetz'
								THEN 'carrie.graetz@kfcu.org'
								ELSE RTRIM(REPLACE(FunderMgr.Summary, '|', '')) 
								END
							)
							,'LADONNA.SPIERS@KFCU.ORG'
							,'LADONNA.DANIELS@KFCU.ORG'
				)
			) 
			as 'FunderAORManagerEmail'
	 ,	UPPER( 
				REPLACE(
							(
								CASE WHEN Funder.ReportsToName like 'Carrie Graetz'
